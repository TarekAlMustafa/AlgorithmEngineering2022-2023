...
int main() {
    ...
    for(unsigned char *p = gray_img, *pg = MM_img; p != gray_img + gray_img_size - (width*9 + window*window - 1); p ++ , pg ++) { //find correct end of for loop, start was -2*width+2
    ...
    // leftover rows
    for(unsigned char *p = gray_img + gray_img_size - (width*9 + window*window - 1), *pg = MM_img + gray_img_size - (width*9 + window*window - 1), j = 0; p < gray_img + gray_img_size; p ++ , pg ++, j ++){
        int h = (int)j / width;
        for(int i = 0; i < window*window; i++){
